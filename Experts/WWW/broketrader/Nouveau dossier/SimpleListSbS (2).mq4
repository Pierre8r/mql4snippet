...
int OnInit() {
   ...
   // Let's now remove some objects and print new count
   MyList.Remove( Second );
   MyList.Remove( First );
   Print( MyList.Count() );

   // Add them again and use of the indexing operator   
   MyList.Insert( MyList.Begin(), Second );
   MyList.Insert( MyList.Begin(), First );

   int ListCount = MyList.Count();
   for( int i=0; i<ListCount; i++ ){
      // We have to cast ListNode* to ListedObject* in order to use the Name()method
      Print( ((ListedObject*)(MyList[i])).Name() );
   }
